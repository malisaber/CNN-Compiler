...
public:
	...
	virtual bool Map_Data_Blocks_and_Build_Dependencies(Data_Logger* DataL,	Dependency_Logger* DpndL) = 0;
	...
	virtual bool Get_output_scheduling_info(SNID_t****& PLONM, DBID_t****& PLODM, Conv_Layer_Info& map_entity_info, bool& GFI) = 0;
	...
	virtual bool Get_previous_output_scheduling_info(Layer_2D* prev_layer);
};
...
public:
	...
	Input_Layer_2D(Conv_Layer_Info Linfo, Conv_Layer_Info Maxes, size_t layer_id);
	~Input_Layer_2D();
	...
	bool Map_Data_Blocks_and_Build_Dependencies(Data_Logger* DataL, Dependency_Logger* DpndL) override;

	//	This function returns the scheduling info of the output of this layer.
	bool Get_output_scheduling_info(SNID_t****& PLONM, DBID_t****& PLODM, Conv_Layer_Info& map_entity_info, bool& GFI) override;
...
public:
	...
	Conv_MKMC_2D(Conv_Layer_Info Linfo, Conv_Layer_Info Maxes, size_t layer_id);
	~Conv_MKMC_2D();
...
public:
	...
	MPDR_MC_2D(Conv_Layer_Info Linfo, Conv_Layer_Info Maxes, MaxP_Layer_Info Window, size_t layer_id);
	~MPDR_MC_2D();
...
public:
	...
	Output_Layer_2D(Conv_Layer_Info Linfo, Conv_Layer_Info Maxes, size_t layer_id);
	~Output_Layer_2D();
...
public:
	...
	size_t	Add_Layer				(Layer_2D_Types ltype, Conv_Layer_Info linfo, Conv_Layer_Info lmaxes);
	
	// Add a layer with a specific max-pooling window size.
	size_t	Add_Layer				(Layer_2D_Types ltype, Conv_Layer_Info linfo, Conv_Layer_Info lmaxes, MaxP_Layer_Info MP_window);
	...
	bool	Build_Network			(Data_Logger* DataL, Dependency_Logger* DpndL);

	// Return maxes info for a given layer.
	Conv_Layer_Info Get_Maxes_info	(size_t id);

	// return the input sizes 
	void	Get_input_sizes			(std::vector<Conv_Layer_Info>& info, std::vector<Conv_Layer_Info>& minfo, std::vector<size_t>& lid);

	// return the weight sizes
	void	Get_weight_sizes		(std::vector<Conv_Layer_Info>& info, std::vector<Conv_Layer_Info>& minfo, std::vector<size_t>& lid);

private:
	// Internal layer factory that instantiates the requested derived type.
	size_t base_Add_Layer			(Layer_2D_Types ltype, Conv_Layer_Info linfo, Conv_Layer_Info lmaxes, MaxP_Layer_Info MP_window);
};
