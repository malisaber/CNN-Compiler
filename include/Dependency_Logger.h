...
public:
	...
	bool print_Scheduling_Node_Info					(
														SNID_t SNI);
	...
	size_t print_Thread_info_file					(
														std::filesystem::path name);

	// Dump optimized thread info to a text file (appends .txt).
	size_t print_Optimized_Execution_Thread_info_file(
														std::filesystem::path name);
	...
	size_t Calculate_Data_Block_Timing				(
														Data_Logger*	DataL);
...
private:
	...
	SNID_t	Douplicate_and_cleanse_Scheduling_Node	(
														SNID_t SNI,
														SNID_t NSNI);
	...
	bool	Fetch_first_Scheduling_Nodes_with_Dependency(
														SNID_t& NID,
														Dependency dep,
														size_t lvl);

	// Calculate required space counts and offsets for allocation.
	void	Calculate_Required_Spaces				(
														Data_Logger* DataL);
	...
	size_t	Get_Max_seq_length_of_level				(
														size_t lvl);
