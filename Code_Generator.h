...
public:
	...
	bool	Extract_PE_Execution_Info				(Dependency_Logger* DpndL);
	...
	bool	Code_Wizard								(Dependency_Logger* DpndL, Data_Logger* DataL, NETWORK* network, std::filesystem::path Src_fname, std::filesystem::path dst_fname, std::filesystem::path CGCW_Dump_dir);
...
private:
	...
	bool	Extract_Single_PE_Execution_Info		(Dependency_Logger* DpndL, CG_PE_Node& PEInfo, size_t lvl, size_t baseline, size_t plane, size_t vault);

	// Build MPDR execution info for a single baseline.
	bool	Extract_MPDR_Execution_Info				(Dependency_Logger* DpndL, CG_MPDR_Node& MPDRInfo, size_t lvl, size_t baseline);

	// Link activation/accumulation outputs back into the owning PE entries.
	void	Link_PE_Storage							(Dependency_Logger* DpndL, size_t lvl, size_t baseline);
	...
	void	Modify_MPDR_units						(Data_Logger* DataL);

	// Reorder MPDR units for a specific level.
	void	Modify_MPDR_units						(Data_Logger* DataL, size_t lvl);

	// Check if PE inputs/outputs/accumulators are equally spaced in memory.
	void	Equal_Spaced_Check_PE					(Data_Logger* DataL, size_t lvl, size_t bline, size_t node, bool& ESI, bool& ESO, bool& ESP);

	// Check if MPDR inputs/outputs are equally spaced in memory for a baseline.
	void	Equal_Spaced_Check_MPDR					(Data_Logger* DataL, size_t lvl, size_t bline);
	
	// Adjust zero-block inputs to the closest duplicates with correct spacing.
	void	Modify_Zero_Blocks						(Data_Logger* DataL, size_t lvl, size_t bline, size_t node, bool& first,  bool& last);

	// Apply all normalization/spacing/zero-block fixes and dump intermediate files.
	void	Modify_All								(Data_Logger* DataL, std::filesystem::path fname);

	// Emit a spacing diagnostics report file.
	void	Print_Spacing							(Data_Logger* DataL, std::filesystem::path fname);
	...
	// Generate Codes
	void	Generate_Codes							(Dependency_Logger* DpndL, Data_Logger* DataL, NETWORK* network, std::filesystem::path srce, std::filesystem::path fname);

	// Copy template source files from srce to dest (overwrite if present).
	void	Copy_File								(std::filesystem::path srce, std::filesystem::path dest);

	// Emit the first part of generated main.cpp (includes globals and setup).
	void	Generate_Main_P1						(std::ofstream& files_out);

	// Emit "Platform_Execute_Layer_<lvl>" function body.
	void	Generate_Platform_Execute_Layer_lvl		(Data_Logger* DataL, std::ofstream& files_out, size_t lvl);

	// Emit "Platform_Execute_Layer_<lvl>_BL<bline>" function body.
	void	Generate_Platform_Execute_Layer_bl		(Data_Logger* DataL, std::ofstream& files_out, size_t lvl, size_t bline);

	// Emit "Platform_Execute_BseLine" function body.
	void	Generate_Platform_Execute_BseLine		(std::ofstream& files_out);

	// Emit "Platform_Peripheral_Layer_<lvl>" function body.
	void	Generate_Platform_Peripheral_Layer_lvl	(Dependency_Logger* DpndL, Data_Logger* DataL, NETWORK* network, std::ofstream& files_out, size_t lvl);

	// Emit the tail of generated main.cpp (interrupt handlers and helpers).
	void	Generate_Main_P2						(std::ofstream& files_out);

	// adding necessary data for execution part to the Data.h and making its array
	void	Generage_Data_Blocks_Exe_lvl_bline		(Data_Logger* DataL, size_t lvl, size_t bline);

	// adding necessary data for peripheral part to the Data.h and making its array
	void	Generage_Data_Blocks_Peri_lvl			(Dependency_Logger* DpndL, Data_Logger* DataL, NETWORK* network, size_t lvl, std::vector<std::vector<size_t>> All_BLs);
	
	// adding datablocks
	void	Generage_Data_Blocks_Exe_Baseline		(Data_Logger* DataL);
};
